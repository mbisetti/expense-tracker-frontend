...
const installmentsRec: RecurringExpense = {
  ...
  billingDay: 5, weekday: null, dueMonth: null, installmentsTotal: 12, cashPrice: 10000,
  autoDebit: false, debitAccountId: null, debitPaymentMethodId: null, active: true,
...
const historyTx = {
  ...
  createdAt: '2026-06-05T00:00:00', recurringExpenseId: 'rec1', autoGenerated: true,
};

// Recurrente con débito automático + item con débitos fallidos (para la caja + reintento).
const autoRec: RecurringExpense = {
  ...installmentsRec, id: 'rec1', autoDebit: true, debitAccountId: 'acc1', debitPaymentMethodId: null,
};
const failedItem: RecurringItem = { ...item, failedCount: 2 };

let deleteCalls: string[];
let retryResult: { executed: number; stillFailed: number };

beforeEach(() => {
  deleteCalls = [];
  retryResult = { executed: 1, stillFailed: 0 };
  vi.stubGlobal(
    'fetch',
    vi.fn((url: string, options?: RequestInit) => {
      ...
      if (options?.method === 'POST' && url.includes('/debits/retry')) {
...
function renderDetail(recurring: RecurringExpense, itemOverride: RecurringItem = item) {
  const onClose = vi.fn();
  ...
  render(
    <QueryClientProvider client={queryClient}>
      <AuthContext.Provider value={{ accessToken: 't', status: 'authenticated', setAccessToken: () => {} }}>
        <ToastProvider>
          <RecurringDetailModal recurring={recurring} item={itemOverride} onClose={onClose} onEdit={() => {}} />
        </ToastProvider>
...
describe('RecurringDetailModal', () => {
  ...
  it('badge Auto en las filas autogeneradas del historial', async () => {
    renderDetail(installmentsRec);
    expect(await screen.findByText('Cuota 6')).toBeInTheDocument();
    expect(screen.getByText('Auto')).toBeInTheDocument();
  });

  it('caja Sin saldo con reintento exitoso → toast de éxito', async () => {
    retryResult = { executed: 1, stillFailed: 0 };
    renderDetail(autoRec, failedItem);
    fireEvent.click(await screen.findByRole('button', { name: 'Reintentar ahora' }));
    expect(await screen.findByText('Débito realizado.')).toBeInTheDocument();
  });

  it('reintento que sigue sin saldo → toast de error', async () => {
    retryResult = { executed: 0, stillFailed: 1 };
    renderDetail(autoRec, failedItem);
    fireEvent.click(await screen.findByRole('button', { name: 'Reintentar ahora' }));
